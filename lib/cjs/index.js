...
var getCurFiles = function (dir, SuffixIncludes, unFileIncludes) {
    ...
    var filenameList = fs.readdirSync(dir).sort().filter(function (filename) {
        var fileInfo = fs.statSync(path__default["default"].join(dir, filename));
...
var getAllDirs = function (dir, unDirIncludes) {
    ...
    items.forEach(function (item) {
        var dirName = path__default["default"].join(dir, item);
...
var createREADME = function (dir, unDirIncludes, unFileIncludes) {
    ...
    var file = path__default["default"].join(dir, './README.md');
...
var AutoNavPlugin = function (options) {
    ...
    var path$1 = path.resolve(process.cwd(), 'docs');
    return getNav(path$1, 0);
};
